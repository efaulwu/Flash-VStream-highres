# [ICCV 2025] Flash-VStream-highres: Efficient Real-Time Understanding for Long Video Streams at Native Resolution

This is the high-resolution variant of Flash-VStream-Qwen that processes video frames at their **native resolution** instead of the fixed 224×224 pixel constraint. By removing the pixel count cap, the model preserves the full spatial detail of video frames, allowing it to benefit from higher-resolution inputs.

Key differences from Flash-VStream-Qwen:
- `VIDEO_MAX_PIXELS` is set to `int(1e9)` (effectively unlimited) instead of `768 * 28 * 28`
- `VIDEO_TOTAL_PIXELS` is set to `int(1e12)` (effectively unlimited) instead of `24576 * 28 * 28`
- Default `max_pixels` in training (`finetune_flash.py`) is `-1` (native resolution, no cap)
- Default `max_pixels` in inference (`inference_mcq_vqa.py`) is `None` (native resolution)
- The CLI streaming demo (`cli_server_2gpu.py`) no longer applies a `4×224×224` pixel cap
- Training and evaluation scripts no longer pass a fixed `max_pixels` argument

The model architecture (Flash Memory, temporal pooling, spatial enhancement, AM-RoPE) is unchanged and already supports variable resolutions natively through the Qwen2-VL dynamic resolution mechanism. Frames are still rounded to the nearest multiple of `patch_size × merge_size × temporal_poolsize = 56` pixels to satisfy patch embedding requirements.
...
cd Flash-VStream/Flash-VStream-Qwen-highres
```
...
If you want to train Flash-VStream-highres from pretrained LLM and evaluate it, please run the following command:
...
If you want to evaluate a Flash-VStream-highres model, please run the following command:
