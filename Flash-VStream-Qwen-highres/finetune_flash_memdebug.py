...
def install_lazy_dataset_probe(mem_logger: MemoryLogger, interval: int) -> None:
    """Monkey patch LazySupervisedDataset to log cache growth."""
    ...
    def wrapped_getitem(self, index):  # type: ignore[override]
        ...
        if cache is not None:
            ...
            if cache_size > 0 and cache_size % max(1, interval) == 0:
                mem_logger.log_stage(
                    "dataset_cache_growth",
                    component="dataset",
                    extra={"cached_samples": cache_size, "last_index": int(index)},
                )
        return item
...
def train_with_memory_debug() -> None:
    ...
    install_lazy_dataset_probe(mem_logger, mem_args.dataset_cache_log_interval)
