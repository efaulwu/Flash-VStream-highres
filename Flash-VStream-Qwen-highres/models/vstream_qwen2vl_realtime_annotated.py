"""
vstream_qwen2vl_realtime_annotated
=================================

该文件是 `vstream_qwen2vl_realtime.py` 的注释版副本，核心目标是帮助阅读和二次开发：

1) 解释 FlashMemory（时序/空间记忆）在流式视频中的数据流。
2) 解释 realtime 场景下，如何把每个新 chunk 的视觉特征增量写入记忆。
3) 解释在真正 LLM 前向前，如何从记忆恢复 video_embeds 与 AM-RoPE 位置编码。

注意：
- 本文件保持与原实现相同的执行逻辑，仅增加注释和文档字符串。
- 如需验证功能一致性，建议与原文件并行运行同一测试。
"""
...
# 计算 memory 压缩后在 LLM 视角下的有效视频网格尺寸。
# 输入 thw 是视觉分支 token 网格，输出 real_thw 是 memory 参与 LLM 的 token 网格。
...
class FlashMemory(nn.Module):
    ...
    整体分两步：
    - temporal_compress：把长时序压到固定窗口（CSM）。
    - spatial_enhance：根据 CSM 从原特征库检索补充空间细节（DAM）。

    最终 `cat_spa_tem` 会把 DAM + CSM 串接给后续 merger。
    """
    ...
    """计算 CSM（Compressed Sequential Memory）。

    输入：完整时序特征 x。
    输出：压缩后时序特征 tem_x，及其时间戳/权重信息（用于后续空间检索）。
    """
    ...
    """基于 CSM 从原始特征库检索 DAM（Detail-Aware Memory）。

    直观理解：
    - tem_x 是“概览记忆”（压缩后的关键时刻）。
    - spa_x 是“细节补充”（从原特征里挑回高价值帧）。
    """
    ...
    """AM-RoPE: 按 CSM + DAM 的真实时间位置重算视觉 3D RoPE。

    由于 memory 会重排/压缩视觉 token，原始位置编码不再准确，
    因此这里依据 `tem_positions` 与 `spa_positions` 重建视觉 token 的时空索引。
    """
...
class FlashVStreamQwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def forward_simple_not_merge(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        # realtime 路径会先调用该函数得到视觉 backbone 特征，不做 memory merge。
        # 后续在 `embed_new_video_clip` 里显式完成 temporal/spatial memory 更新。
...
class FlashVStreamQwen2VLModel(Qwen2VLForConditionalGeneration):
    ...
    def __init__(self, config): 
        ...
        # streaming 开关（历史代码保留了 model/mode 命名，实际使用 mode 字段）
    ...
    def embed_new_video_clip(self, pixel_values_videos, video_grid_thw, start_idx):
        """把一个新 video chunk 增量写入流式记忆。

        关键流程：
        1) 提取该 chunk 的视觉特征（不 merge）。
        2) 与旧记忆拼接，执行 temporal_compress + spatial_enhance。
        3) 生成最新 video_embeds 并回写共享内存（供前向读取）。

        返回值是关键阶段耗时时间戳，便于 profiling。
        """
...
# self.visual.flash_memory
    ...
    def prepare_realtime_inference(self, position_ids, visual_position_ids):
        """在 LLM 前向前准备 realtime 输入。

        从共享记忆读取：
        - 最新 video_embeds（用于替换 `<video>` token 的 embedding）。
        - 基于记忆中实际时间位置重算后的 AM-RoPE position_ids。
        """
